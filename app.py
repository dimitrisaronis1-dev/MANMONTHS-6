...
def process_allocation(uploaded_input_file, template_workbook):
    ...
    for r_clear in rows_to_clear_completely:
        for c_clear in range(1, max_col_to_clear):
            ...
            ws.cell(r_clear, c_clear).border = Border() # Explicitly reset border
    ...
    for y in years:
        year_start_col = col
        year_end_col = col + 11 # For 12 months, the end column is 11 positions after start

        # Ensure all cells in the year header row range have a border *before* merging
        for c_init_border in range(year_start_col, year_end_col + 1):
            ws.cell(YEAR_ROW, c_init_border).border = thin_border

        year_header_cell = ws.cell(YEAR_ROW, year_start_col) # Now this cell already has a border
        ...
        # This part ensures MONTH_ROW also has borders.
        for m in range(1,13):
